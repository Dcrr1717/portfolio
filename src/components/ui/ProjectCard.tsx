...
export const ProjectCard = component$<{ project: Project; wide?: boolean }>(({ project, wide = false }) => {
    ...
    >
      ...
      <div class="relative">
        <div class="flex items-center gap-3">
          ...
          <span class="font-mono text-[10px] uppercase tracking-[0.16em] text-ink-muted">{tr(project.category, l)}</span>
        </div>
        <h3 class={`mt-5 font-display font-semibold leading-tight tracking-tight text-ink ${wide ? "text-3xl sm:text-4xl" : "text-2xl"}`}>
          {tr(project.title, l)}
        </h3>
        <p class={`mt-3 max-w-md text-ink-secondary ${wide ? "text-base" : "text-sm"}`}>{tr(project.oneLiner, l)}</p>
      </div>

      <div class="relative mt-8">
        <div class="mb-5 flex flex-wrap gap-1.5">
          {chips.map((t) => (
            <span key={t} class="rounded-full border border-line bg-surface-2 px-2.5 py-1 font-mono text-[11px] text-ink-secondary">
              {t}
            </span>
          ))}
        </div>
